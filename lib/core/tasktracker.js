    ...
    switch (field.toLowerCase()) {
      ...
      case 'remove-file':
        ...
        if (task.relatedFiles) {
          ...
          const normalizedRemovePath = path.normalize(fileToRemove);
          
          // Find normalized path in related files
          const index = task.relatedFiles.findIndex(f => 
            path.normalize(f) === normalizedRemovePath
          );
          
          if (index !== -1) {
            task.relatedFiles.splice(index, 1);
          } else {
            output(`⚠️ File "${fileToRemove}" not found in task's related files`);
            updated = false;
          }
        } else {
          output('⚠️ Task has no related files');
          updated = false;
        }
        break;
      ...
      default:
        ...
        output('Valid fields: status, category, title, description, priority, effort, comment, add-file, remove-file');
        return;
    }
...
function viewTask(taskId) {
  try {
    ...
    if (globalOptions.minimal) {
      ...
      output(`#${task.id}: ${task.title} [${task.status}] [${task.category}]`);
      if (task.description) {
      ...
      if (task.relatedFiles && task.relatedFiles.length > 0) {
        output(`Files: ${task.relatedFiles.join(', ')}`);
      }
      if (task.comments && task.comments.length > 0) {
        output(`Comments: ${task.comments.length}`);
        task.comments.forEach(comment => {
          const date = new Date(comment.date).toLocaleString();
          output(`[${date}] ${comment.author}: ${comment.text}`);
        });
      }
      return;
    }
