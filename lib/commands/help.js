...
function showHelp(commandName, options = {}) {
  try {
    // Get commands safely with fallback for when commands are not properly initialized
    const commands = commandRegistry.commands || {};
    
    if (commandName && commandName !== 'help') {
      return showCommandHelp(commandName, options, commands);
    }
    ...
    Object.entries(groupedCommands).forEach(([group, cmdList]) => {
      ...
      cmdList.forEach(cmd => {
        ...
        if (command) {
          ...
          // Handle case where command definition is missing
          output(`  ${cmd.padEnd(15)} Command not available`, 'info', { globalOptions: options });
        }
      });
    });
 ...
 * @param {object} options Command options
 * @param {object} commands Commands registry object
 * @returns {object} Success status
 */
function showCommandHelp(commandName, options = {}, commands = {}) {
  try {
    // Ensure we have a commands object, even if empty
    commands = commands || {};
    ...
    if (command.alias) {
      ...
      return showCommandHelp(command.alias, options, commands);
    }
