...
if command -v tt >/dev/null 2>&1; then
  ...
  tt j "Git $COMMIT_TYPE: $COMMIT_MSG" --type progress --tags "git,$COMMIT_TYPE,$BRANCH" --files "$(git diff --name-only HEAD~1 HEAD | tr '\\n' ',')"
  ...
  if [ -f ".tasktracker/git-auto" ]; then
    ...
    case $RESPONSE in
      [Yy]* )
        ...
        CONTEXT=$(cat)
        if [ ! -z "$CONTEXT" ]; then
          tt j "$CONTEXT" --type context --tags "git,$COMMIT_TYPE,$BRANCH"
        fi
        ;;
      [Ss]* )
