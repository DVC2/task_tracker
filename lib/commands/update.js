 ...
 * @param {string[]|string} values New values for the field
...
function updateTask(taskId, field, values, options = {}) {
  try {
    ...
    // Ensure values is an array before using it
    const valueArray = Array.isArray(values) ? values : [values];
    
    // Combine values into a single string for some fields
    const fieldValue = valueArray.join(' ');
    
    // First value for status and other field types
    const firstValue = valueArray[0] ? String(valueArray[0]).toLowerCase() : '';
