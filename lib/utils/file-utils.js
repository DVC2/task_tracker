...
async function countFileLines(filePath) {
  return new Promise((resolve) => {
    ...
    try {
      ...
      rl.on('close', () => {
        // Ensure stream is closed on successful completion if not already handled by readline
        if (!readStream.destroyed) {
            readStream.close();
        }
        resolve(lineCount);
      });
      rl.on('error', (err) => { // Capture the error object
        console.warn(`Warning: Error reading file for line count ${filePath}: ${err.message}`);
        if (!readStream.destroyed) {
            readStream.destroy(); // Destroy stream on error
        }
        resolve(0);
      });
      // Catch errors on the stream itself, which might not propagate to readline interface's error event
      readStream.on('error', (err) => {
        console.warn(`Warning: Read stream error for ${filePath}: ${err.message}`);
        if (!readStream.destroyed) {
            readStream.destroy();
        }
        // Ensure promise is resolved if readline error/close hasn't already
        // This might need a flag to prevent double resolve, but resolve(0) is idempotent here if already resolved.
        resolve(0);
      });
    } catch (error) {
...
async function ensureDirectories(directories) {
  ...
  await Promise.all(dirs.map(async (dir) => {
    // No need to check for existence first, fs.promises.mkdir with recursive: true handles it.
    await fs.promises.mkdir(dir, { recursive: true });
  }));
}
