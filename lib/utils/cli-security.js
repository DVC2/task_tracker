 ...
 * @param {string} type Type of argument (command, taskId, filePath, option, value)
...
function sanitizeArgs(args) {
  ...
  // Process command (first arg) if present
  if (args.length > 0) {
  ...
  for (let i = 1; i < args.length; i++) {
    ...
    if (arg.startsWith('-')) {
      ...
      if (i + 1 < args.length && !args[i + 1].startsWith('-')) {
        const valueValidation = validateArg(args[i + 1], 'value');
