  ...
  try {
    ...
    try {
      ...
      if (error.message.includes('not a git repository')) {
        ...
        console.log('ℹ️ Git is not installed or not in PATH. Using file-based tracking instead.');
      } else {
        console.log(`ℹ️ Git integration unavailable: ${error.message.split('\n')[0]}`);
      }
    ...
    if (!fs.existsSync(SNAPSHOTS_DIR)) {
      fs.writeFileSync(SNAPSHOTS_DIR, JSON.stringify([], null, 2));
      console.log(`✅ Created empty ${SNAPSHOTS_DIR}`);
    }
...
function listTasks(statusFilter, showCurrentOnly = false, showFull = false) {
  // Check if TaskTracker is initialized
  if (!fs.existsSync(TASKS_PATH)) {
    console.error(colorize('❌ TaskTracker not initialized! Please run: tasktracker init', 'error'));
    process.exit(1);
  }
  ...
  try {
    ...
    if (taskData.tasks.length === 0) {
      console.log(colorize('📋 No tasks found. Create a task with "tasktracker add" or "tasktracker quick"', 'info'));
      return;
    }
...
function viewTask(taskId) {
  ...
  // Validate task ID
  if (!taskId) {
    console.error('❌ Missing task ID. Usage: tasktracker view <id>');
    process.exit(1);
  }
...
function updateTask(taskId, field, values) {
  ...
  if (!taskId) {
    ...
    console.log('\nExamples:');
    console.log('  tasktracker update 1 status done');
    console.log('  tasktracker update 2 category feature');
    console.log('  tasktracker update 3 priority p1-high');
    console.log('  tasktracker update 4 effort 5-large');
    console.log('  tasktracker update 5 add-file src/app.js');
    console.log('  tasktracker update 5 checklist create "Implementation Steps"');
    console.log('  tasktracker update 5 checklist add 0 "Write unit tests"');
    process.exit(0);
  }
  ...
  try {
    ...
    switch (field) {
      case 'status':
        ...
        if (!newStatus) {
          console.error('❌ Missing status value. Example: tasktracker update 1 status done');
          process.exit(1);
        }
      ...
      case 'priority':
        ...
        if (!newPriority) {
          console.error('❌ Missing priority value. Example: tasktracker update 1 priority p1-high');
          process.exit(1);
        }
      ...
      case 'effort':
        ...
        if (!newEffort) {
          console.error('❌ Missing effort value. Example: tasktracker update 1 effort 5-large');
          process.exit(1);
        }
      ...
      case 'category':
        ...
        if (!newCategory) {
          console.error('❌ Missing category value. Example: tasktracker update 1 category feature');
          process.exit(1);
        }
      ...
      case 'title':
        ...
        if (!newTitle) {
          console.error('❌ Missing title. Example: tasktracker update 1 title "New title"');
          process.exit(1);
        }
      ...
      case 'description':
        ...
        if (!newDescription) {
          console.error('❌ Missing description. Example: tasktracker update 1 desc "New description"');
          process.exit(1);
        }
      ...
      case 'add-file':
        ...
        if (!filePath) {
          console.error('❌ Missing file path. Example: tasktracker update 1 add-file src/app.js');
          process.exit(1);
        }
      ...
      case 'comment':
        ...
        if (!commentText) {
          console.error('❌ Missing comment text. Example: tasktracker update 1 comment "Fixed the issue"');
          process.exit(1);
        }
      ...
      case 'checklist':
        if (!values[0]) {
          console.error('❌ Missing checklist action. Usage: tasktracker update 1 checklist <action>');
          console.log('Actions:');
          console.log('  create "Title"          - Create a new checklist');
          console.log('  add <index> "Item"      - Add item to checklist');
          console.log('  toggle <cIndex> <iIndex> - Toggle item completion');
          console.log('  remove <cIndex> <iIndex> - Remove item from checklist');
          console.log('  delete <index>          - Delete entire checklist');
          process.exit(1);
        }
        ...
        switch(checklistAction) {
          case 'create':
            ...
            if (!title) {
              console.error('❌ Missing checklist title. Example: tasktracker update 1 checklist create "Implementation Steps"');
              process.exit(1);
            }
          ...
          case 'add':
            if (!values[1] || isNaN(parseInt(values[1]))) {
              console.error('❌ Missing or invalid checklist index. Example: tasktracker update 1 checklist add 0 "Write tests"');
              process.exit(1);
            }
...
function findChangedFilesByHash(fileHashes, pathFilter) {
  ...
  try {
    ...
    for (const dir of checkedDirs) {
      ...
      processDirectory(dir, fileHashes, changedFiles, pathFilter, ignoreDirs, ignoreExtensions, 
    ...
    if (checkedDirs.size === 0) {
      processDirectory('.', fileHashes, changedFiles, pathFilter, ignoreDirs, ignoreExtensions, 
        { filesChecked, maxFilesToCheck }, 
        (count) => { filesChecked = count; }
      );
    }
...
function processDirectory(dirPath, fileHashes, changedFiles, pathFilter, ignoreDirs, ignoreExtensions, countRef, updateCount) {
  try {
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (stats.isDirectory()) {
          ...
          processDirectory(filePath, fileHashes, changedFiles, pathFilter, ignoreDirs, ignoreExtensions, countRef, updateCount);
