...
function initializeTaskTracker() {
  ...
  // Use the showQuickStartGuide function instead of duplicating code
  showQuickStartGuide();
...
function updateTask(taskId, field, values) {
  ...
  try {
    ...
    switch (field) {
      ...
      case 'checklist':
        if (!values[0]) {
          console.error('❌ Missing checklist action. Usage: ./bin/tasktracker update 1 checklist <action>');
          console.log('Actions:');
        ...
        switch(checklistAction) {
          case 'create':
            ...
            if (!title) {
              console.error('❌ Missing checklist title. Example: ./bin/tasktracker update 1 checklist create "Implementation Steps"');
              process.exit(1);
            }
          ...
          case 'add':
            if (!values[1] || isNaN(parseInt(values[1]))) {
              console.error('❌ Missing or invalid checklist index. Example: ./bin/tasktracker update 1 checklist add 0 "Write tests"');
              process.exit(1);
            }
            ...
            if (!itemText) {
              console.error('❌ Missing item text. Example: ./bin/tasktracker update 1 checklist add 0 "Write tests"');
              process.exit(1);
            }
          ...
          case 'toggle':
            if (!values[1] || isNaN(parseInt(values[1])) || !values[2] || isNaN(parseInt(values[2]))) {
              console.error('❌ Missing or invalid indices. Example: ./bin/tasktracker update 1 checklist toggle 0 2');
              process.exit(1);
            }
          ...
          case 'remove':
            if (!values[1] || isNaN(parseInt(values[1])) || !values[2] || isNaN(parseInt(values[2]))) {
              console.error('❌ Missing or invalid indices. Example: ./bin/tasktracker update 1 checklist remove 0 2');
              process.exit(1);
            }
          ...
          case 'delete':
            if (!values[1] || isNaN(parseInt(values[1]))) {
              console.error('❌ Missing or invalid checklist index. Example: ./bin/tasktracker update 1 checklist delete 0');
              process.exit(1);
            }
