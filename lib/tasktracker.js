...
// Format text with colors based on status or category
  ...
  try {
    if (type === 'status') {
      ...
      if (text.startsWith('1') || text.startsWith('2')) {
        return chalk.green(text);
      } else if (text.startsWith('3') || text.startsWith('5')) {
        return chalk.yellow(text);
      } else if (text.startsWith('8') || text.startsWith('13')) {
        return chalk.red(text);
      } else {
        return chalk.white(text);
      }
    ...
    console.error(`Warning: Color formatting error: ${error.message}`);
    return text;
  }
...
function updateTask(taskId, field, values) {
  ...
  if (!taskId) {
    ...
    console.log('  addfile    - Add related file to task');
  ...
  try {
    ...
    switch (field) {
      ...
      case 'addfile':
        ...
        if (!filePath) {
          console.error('❌ Missing file path. Example: ./bin/tasktracker update 1 addfile src/app.js');
          process.exit(1);
        }
      ...
      default:
        ...
        console.log('Valid fields: status, category, title, desc, addfile, comment');
        process.exit(1);
    }
...
function trackChanges(pathFilter) {
  ...
  try {
    ...
    try {
      ...
      console.log('⚠️ Git not available, using file hash tracking instead.');
      
      // Manual file tracking not implemented in this demo
      // Would require walking the directory tree and comparing file hashes
      console.log('⚠️ Manual file hash tracking not implemented in this demo version.');
      changedFiles = [];
    }
    ...
    changedFiles.forEach(file => {
      try {
        // For a real implementation, we would calculate and store file hashes here
        fileHashes[file] = {
